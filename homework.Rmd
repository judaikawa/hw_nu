...
We could additionally look at how it performns as a binary classifier, with metrics like Recall, Precision, F1-Score and Confusion Matrix. We do however would need to define a threshold on this case.
...
```{r}
full <- default ~ score_1 + score_2 + score_3 + score_4 + score_5 + score_6 + risk_rate + amount_borrowed + borrowed_in_months + credit_limit + income + gender + facebook_profile + real_state + n_bankruptcies + n_defaulted_loans + n_accounts
...
print(paste("R squared: ",round( 1 - ( summary(glm)$deviance / summary(glm)$null.deviance ), 2 ))) # R squared
```

### ROC and AUC
```{r}
pred <- predict(glm, validation, type='response')

roc_auc <- function(pred) {
  metrics <- ROCR::prediction(pred, validation$default)
  ## ROC
...
logLoss(pred[-which(is.na(pred))], ifelse(validation$default==0,0,1)[-which(is.na(pred))])
```

The Logistic Regression is known for working better with categorical variables, so let's build try it out. A decision tree was built to every variable in the dataset and its splits were made to categorize the numerical variables and reduce the levels of the categories with high dimensionality.
```{r}
binary_data <- data.frame(sample_data$ids)

for (var in c('score_3','score_4')) {
...
```{r}
pred <- predict(rf, validation, type='prob')
pred <- pred[,2]
roc_auc(pred)
```
